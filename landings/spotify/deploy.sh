...
then
    ...
    echo "docker tag ${_CONTAINER_NAME}:${_TAG} ${_REPO_URL}/${_CONTAINER_NAME}:${_TAG}"
    docker tag ${_CONTAINER_NAME}:${_TAG} ${_REPO_URL}/${_CONTAINER_NAME}:${_TAG}
    echo "docker push ${_REPO_URL}/${_CONTAINER_NAME}:${_TAG}"
...
then
  ...
  if [ -x './ops/scripts/aws-stack-check.sh' ]
  then
    cd ops/scripts/
